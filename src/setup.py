def setup():
    ...
    if is_platform_windows():
        ...
        pyVersion = (
            str(plf.python_version())[:index]
            + str(plf.python_version())[index + 1 : index + 3]
        )
        innerTube = str(
            user_directory
        ) + "\\AppData\\Local\\Programs\\Python\\Python{}\\Lib\\site-packages\\pytube\\innertube.py".format(
            pyVersion
        )
    else:
        pyVersion = str(plf.python_version())[: index + 3]
        innerTube = "/usr/local/python/{}/lib/python{}/site-packages/pytube/innertube.py".format(
            plf.python_version(), pyVersion
        )
    #####################################################################################################
